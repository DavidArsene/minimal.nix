  ...
  man-eater =
    ...
    if pkg.meta ? outputsToInstall then
      pkg.overrideAttrs (prev: {
        meta =
          let
            oldMeta = prev.meta or (warn "[man-eater] ${pkg.name}\t has no prev.meta" pkg.meta);
      ...
      warn "[man-eater] ${pkg.name}\t has no meta.outputsToInstall" pkg;
...

{
  ...
  config = {
    ...
    # Set this here so that it has the right priority and allows ergonomic merging.
    ...
    system.path = builtins.trace "Running man-eater..." pkgs.buildEnv {
